...
- [Capítulo 0 — Modelo mental de Git](#capítulo-0--modelo-mental-de-git)
- [Capítulo 1 — Nace el repositorio: `git init`](#capítulo-1--nace-el-repositorio-git-init)
- [Capítulo 2 — Estructura profesional del repositorio](#capítulo-2--estructura-profesional-del-repositorio)
- [Capítulo 3 — Licencias: elegir bien](#capítulo-3--licencias-elegir-bien)
- [Capítulo 4 — El primer commit (y las herramientas de inspección)](#capítulo-4--el-primer-commit-y-las-herramientas-de-inspección)
- [Capítulo 5 — Ramas (branches)](#capítulo-5--ramas-branches)
- [Capítulo 6 — Trabajo diario: modificar, mover, borrar, comparar](#capítulo-6--trabajo-diario-modificar-mover-borrar-comparar)
- [Capítulo 7 — Conectar con GitHub: remoto y `push`](#capítulo-7--conectar-con-github-remoto-y-push)
- [Capítulo 8 — GitHub desde la terminal: `gh` (GitHub CLI)](#capítulo-8--github-desde-la-terminal-gh-github-cli)
- [Capítulo 9 — Pull Requests: el flujo completo](#capítulo-9--pull-requests-el-flujo-completo)
- [Capítulo 10 — Versiones: Tags y Releases](#capítulo-10--versiones-tags-y-releases)
- [Chuleta copiar-pegar (el 95% de las veces)](#chuleta-copiar-pegar-el-95-de-las-veces)
- [Tabla de rescate rápido (cuando algo sale mal)](#tabla-de-rescate-rápido-cuando-algo-sale-mal)
- [Glosario](#glosario)
...
### Ejemplo `website-achalma`

| Archivo                            | Para qué sirve                                                        | ¿En este repo?    |
| ---------------------------------- | --------------------------------------------------------------------- | ----------------- |
| `README.md`                        | Portada: qué es, cómo instalar/usar. Lo primero que se lee.           |                   |
...
### Qué ocurre internamente — el `.gitignore`
...
### Ejemplo — la doble licencia de este proyecto
...
git commit -m "docs: add project README"   # guardar la commit (foto) con mensaje
git log --oneline -10      # ver las últimas 10 commits (fotos) (resumidas)
...
git checkout main && git pull    # antes de crear una rama (trae cambios remotos)
...
### Ejemplo — convención de nombres de este proyecto

Formato: **`<tipo>/<nombre-corto-con-guiones>`**, en minúsculas.

| Prefijo     | Cuándo                                 | Ejemplo / posible                           |
| ----------- | -------------------------------------- | ------------------------------------------- |
...
### Qué ocurre internamente — `git mv` vs `mv`
...
### Conflictos sencillos (cuando dos ramas tocan la misma línea)
...
### Ejemplo — Issues (tareas/bugs en GitHub)
...
### Formas de integrar (merge) — comparación
...
### Ejemplo — el ciclo de vida del PR #10 (de principio a fin)
...
### Anatomía de un buen cuerpo de PR
...
## Capítulo 9.1 — Merge local: integrando cambios sin GitHub

### Objetivo

Integrar una rama de trabajo en `main` de forma local, decidiendo la estrategia de fusión más adecuada. Este flujo aplica cuando trabajas sin repositorio remoto o cuando quieres probar la integración antes de abrir un PR.

### Concepto

El **merge local** es la operación manual de `git merge` en tu terminal. Sin intermediarios (sin Pull Request), tú eres quien decide cuándo y cómo fusionar. Es la base del flujo de trabajo con ramas, incluso si luego usas PRs en la nube.

### Estrategias de merge — comparativa ampliada

| Estrategia            | Comando                                               | Qué hace                                             | Cuándo usarla                                 |
| --------------------- | ----------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------- |
| **Merge commit**      | `git merge --no-ff`                                   | conserva todos los commits + crea commit de fusión   | historia detallada, flujo con PRs (estándar)  |
| **Squash**            | `git merge --squash`                                  | aplasta todos los commits de la rama en **uno solo** | ramas con muchos "wip", limpieza de historial |
| **Fast-forward (FF)** | `git merge` (por defecto)                             | si `main` no avanzó, solo mueve el puntero           | ramas simples sin bifurcación                 |
| **No-FF**             | `git merge --no-ff`                                   | fuerza commit de fusión aunque sea FF                | documentar en el historial que hubo una rama  |
| **Rebase + merge**    | `git rebase main` + `git checkout main` + `git merge` | historial lineal sin commits de fusión               | equipos que prefieren historia plana          |

---

### Ejemplo — el ciclo de vida de `feature/actualizar-monografia-eeuu` (local)

```mermaid
flowchart TD
    A["git checkout -b feature/actualizar-monografia-eeuu"] --> B["...ediciones en LaTeX..."]
    B --> C["git add ..."]
    C --> D["git commit -m 'eliminar: anexos y presentación'"]
    D --> E["git commit -m 'mejora: actualización contenido'"]
    E --> F["git commit -m 'chore: ignorar PDFs'"]
    F --> G["git checkout main"]
    G --> H{"¿Estrategia?"}
    H --> I["Merge commit: git merge --no-ff"]
    H --> J["Squash: git merge --squash"]
    H --> K["FF: git merge (si no hay cambios en main)"]
```

### Caso práctico: los commits de la monografía

**Commits generados:**

```
dcecfa1 eliminar: anexos y presentación obsoletos de monografía EEUU
a6d3943 mejora: actualización de contenido y referencias monografía EEUU
abc1234 chore: ignorar archivos PDF y artefactos de compilación LaTeX
```

#### Opción A: Merge commit (mantiene los 3 commits)

```bash
# Situación: estabas en feature/actualizar-monografia-eeuu
git checkout main
...
```
cf27fce Merge: integración actualización monografía EEUU
dcecfa1 eliminar: anexos y presentación obsoletos de monografía EEUU
a6d3943 mejora: actualización de contenido y referencias monografía EEUU
abc1234 chore: ignorar archivos PDF y artefactos de compilación LaTeX
```

#### Opción B: Squash (un solo commit en main)

```bash
git checkout main
git merge --squash feature/actualizar-monografia-eeuu
git commit -m "mejora: actualización completa monografía crecimiento económico EEUU 1950-2019

- Eliminación de anexos y presentación obsoletos
- Revisión y mejora de capítulos 1 y 2
- Actualización de introducción y conclusiones
- Correcciones en referencias bibliográficas
- Configuración de .gitignore para artefactos LaTeX"
```

**Resultado en `git log` de `main`:**

```
def4567 mejora: actualización completa monografía crecimiento económico EEUU 1950-2019
```

#### Opción C: Fast-forward (solo si main no cambió)

```bash
git checkout main
git merge feature/actualizar-monografia-eeuu  # por defecto FF si es posible
```

**Resultado:** `main` apunta a `abc1234` (el último commit de la feature), como si nunca hubiera existido una rama separada.
...
```bash
git merge <rama>                    # FF si posible, sino merge commit
git merge --no-ff <rama>            # siempre merge commit
git merge --squash <rama>           # aplasta cambios, no hace commit automático
git merge --abort                   # abortar merge con conflictos
git merge --continue                # continuar después de resolver conflictos
```

### Limpieza post-merge

```bash
git branch -d feature/actualizar-monografia-eeuu   # eliminar rama local
git push origin --delete feature/actualizar-monografia-eeuu  # eliminar remota (si existe)
```

### Comparativa: merge commit vs squash

| Aspecto              | Merge commit                             | Squash                                 |
| -------------------- | ---------------------------------------- | -------------------------------------- |
| **Historial**        | Conserva todos los commits originales    | Los comprime en 1                      |
| **Trazabilidad**     | Se ve exactamente qué se cambió y cuándo | Se pierde el detalle intermedio        |
| **Reversión**        | Puedes revertir un cambio específico     | Revierte todo el cambio o nada         |
| **Tamaño del log**   | Más grande                               | Más limpio y compacto                  |
| **Útil para**        | Desarrollo en equipo, revisión por PR    | Proyectos personales, cambios pequeños |
| **Commits de "wip"** | Quedan visibles (puede ser feo)          | Se ocultan                             |

### Buenas prácticas

1. **Usa `--no-ff`** en equipos para documentar claramente las fusiones
2. **Usa squash** si tu rama tiene muchos commits de "wip", "fix typo", etc.
3. **Nunca hagas squash** si otros ya están trabajando sobre tu rama (reescribe historia)
4. **Verifica con `git diff main..feature`** antes de hacer el merge
5. **Siempre** elimina la rama tras el merge para no acumular basura

### Resumen

```bash
# Paso a paso completo (con squash como ejemplo)
git checkout main
git pull origin main                # si hay remoto, actualizar
git merge --squash feature/actualizar-monografia-eeuu
git commit -m "mejora: actualización monografía EEUU [squash]"
git branch -d feature/actualizar-monografia-eeuu
git status                          # comprobar que todo está limpio
```
...
### Ejemplo (propuesto para este repo)
